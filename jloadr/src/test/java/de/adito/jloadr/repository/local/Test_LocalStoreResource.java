  ...
  public void testHash() throws IOException
  {
    ...
    try
    {
      ...
      try (OutputStream outputStream = resource.getOutputStream();
           DigestingOutputStream digestingOutputStream = new DigestingOutputStream(outputStream);
           InputStream inputStream = new ByteArrayInputStream(data.toString().getBytes()))
      {
        JLoadrUtil.copy(inputStream, digestingOutputStream);
        jlrEntry.setHash(digestingOutputStream.getDigest());
      }
      ...
      try (InputStream inputStream = new ByteArrayInputStream(data.toString().getBytes()))
      {
        ...
        jlrEntry.setHash(JLoadrUtil.calculateHash(resource));
        Assert.assertEquals(hash, resource.getHash());
      }
